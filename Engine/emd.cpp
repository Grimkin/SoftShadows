...
E-Mail: rubner@cs.stanford.edu   URL: http://robotics.stanford.edu/~rubner
*/
...
feature_t Decode( uint32_t code ) {
	feature_t pos;
	pos.x = code;
	pos.y = code >> 1;
	pos.z = code >> 2;
...
std::vector<feature_t> ConvertBrick( uint2 brick ) {
	std::vector<feature_t> out;
	for( uint32_t i = 0; i < 32; i++ ) {
		if( brick.x & 1 << i )
			out.push_back( Decode( i ) );
		if( brick.y & 1 << i )
			out.push_back( Decode( i + 32 ) );
	}
	return out;
}

float dist( feature_t* fA, feature_t* fB ) {
	float dX = static_cast<float>( fA->x - fB->x );
	float dY = static_cast<float>( fA->y - fB->y );
	float dZ = static_cast<float>( fA->z - fB->z );

	return sqrtf( dX * dX + dY * dY + dZ * dZ );
}

float emdBrick( uint2 brickA, uint2 brickB ) {
	std::vector<feature_t> featureA = ConvertBrick( brickA );
	std::vector<float> weightsA;
	weightsA.assign( featureA.size(), 1.0f / featureA.size() );
	std::vector<feature_t> featureB = ConvertBrick( brickB );
	std::vector<float> weightsB;
	weightsB.assign( featureB.size(), 1.0f / featureB.size() );

	signature_t sigA;
	sigA.n = featureA.size();
	sigA.Features = featureA.data();
	sigA.Weights = weightsA.data();
	
	signature_t sigB;
	sigB.n = featureB.size();
	sigB.Features = featureB.data();
	sigB.Weights = weightsB.data();

	return emd( &sigA, &sigB, dist, nullptr, nullptr );
}
...
float emd( signature_t *Signature1, signature_t *Signature2,
	...
	for( XP = _X; XP < _EndX; XP++ ) {
		...
		if( Flow != NULL ) {
			FlowP->from = XP->i;
			FlowP->to = XP->j;
			FlowP->amount = static_cast<float>( XP->val );
			FlowP++;
		}
	}
	if( Flow != NULL )
...
static float init( signature_t *Signature1, signature_t *Signature2,
	...
	return static_cast<float>( sSum > dSum ? dSum : sSum );
}
...
static void newSol() {
	...
	double xMin;
	int steps;
	node2_t *Loop[2 * MAX_SIG_SIZE1], *CurX = nullptr, *LeaveX = nullptr;
...
static void russel( double *S, double *D ) {
	...
	node1_t uHead, *CurU = nullptr, *PrevU = nullptr;
	node1_t vHead, *CurV = nullptr, *PrevV = nullptr;
	node1_t *PrevUMinI = nullptr, *PrevVMinJ = nullptr, *Remember = nullptr;
